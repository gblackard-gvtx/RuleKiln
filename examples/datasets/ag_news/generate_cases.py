...
# 1-indexed in dataset; normalized to lowercase for RuleKiln.
...
def _write_cases_to_file(
    ...
    for i, row in enumerate(rows[:limit]):
        ...
        case: dict[str, object] = {
            ...
            "id": f"ag_news_{target_split}_{i:06d}",
            "split": target_split,
            "task_mode": "classification",
            "input": {"article": str(row["text"])},
            ...
            "metadata": {"source": "ag_news", "label_id": label_id},
            "weight": 1.0,
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate RuleKiln cases for AG News")
    parser.add_argument(
        "--discover-labels",
        action="store_true",
    ...
    full_train = pd.DataFrame(ds["train"])  # type: ignore[index]
    split_idx = int(len(full_train) * 0.90)
    train_rows = full_train.iloc[:split_idx].to_dict("records")
    val_rows = full_train.iloc[split_idx:].to_dict("records")
